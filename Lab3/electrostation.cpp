...
int vehicle_x = 150, vehicle_y = 90;

// Размеры объектов
const int STORAGE_X = 50, STORAGE_Y = 50, STORAGE_W = 100, STORAGE_H = 80;
const int VEHICLE_W = 80, VEHICLE_H = 40;
const int BOILER_X = 400, BOILER_W = 100, BOILER_H = 60;
const int BOILER_Y[4] = {50, 150, 250, 350};

// Позиции остановок
const int STORAGE_STOP_X = STORAGE_X + STORAGE_W + 20;
const int STORAGE_STOP_Y = STORAGE_Y + 20;
const int BOILER_STOP_X = BOILER_X - VEHICLE_W - 20;
...
void MoveVehicleTo(int target_x, int target_y)
{
    ...
    for (int i = 0; i <= steps && run_flag; i++)
    {
        ...
        int new_x = vehicle_x + (target_x - vehicle_x) * progress;
        int new_y = vehicle_y + (target_y - vehicle_y) * progress;
...
void DrawState()
{
    ...
    sprintf(storage_text, "Fuel: %d", (int)fuel_storage.size());
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        // Обновление индикатора уровня топлива
        if (fuel_bar_ids[i] != 0)
        {
            Delete(fuel_bar_ids[i]);
        }
        if (boiler_fuel_level[i] > 0)
        {
            int fuel_height = (boiler_fuel_level[i] * 40) / 20;
            fuel_bar_ids[i] = Rect(BOILER_X + 10, BOILER_Y[i] + BOILER_H - fuel_height, 20, fuel_height, 0,
                                   boiler_states[i] == BURNING ? RGB(255, 100, 0) : RGB(0, 200, 0));
        }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        text_ids[3 + i] = Text(200, y_offset + i * 20, boiler_mark_text, RGB(255, 255, 255));
    }
...
void *VehicleThread(void *arg)
{
    while (run_flag)
    {
        ...
        {
            // Движение к котлу
            int target_y = BOILER_Y[vehicle_target_boiler] + 20;
            MoveVehicleTo(BOILER_STOP_X, target_y);
...
int main()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        boiler_ids[i] = Rect(BOILER_X, BOILER_Y[i], BOILER_W, BOILER_H, 5, RGB(200, 100, 100));
        char boiler_name[20];
    ...
    // Инструкция
    Text(10, 500, "Power Station Simulation - Press 'q' to quit", RGB(255, 255, 255));
